...
public class CustomCircularProgressView extends View {
    ...
    private void initAttributes(AttributeSet attrs, int defStyle) {
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.CustomCircularProgressView, defStyle, 0);

        Resources resources = getResources();

        // Initialize attributes from styleable attributes
        currentProgress = a.getFloat(R.styleable.CustomCircularProgressView_cpv_progress,
                resources.getInteger(R.integer.cpv_default_progress));
        maxProgress = a.getFloat(R.styleable.CustomCircularProgressView_cpv_maxProgress,
                resources.getInteger(R.integer.cpv_default_max_progress));
        thickness = a.getDimensionPixelSize(R.styleable.CustomCircularProgressView_cpv_thickness,
                resources.getDimensionPixelSize(R.dimen.cpv_default_thickness));
        isIndeterminate = a.getBoolean(R.styleable.CustomCircularProgressView_cpv_indeterminate,
                resources.getBoolean(R.bool.cpv_default_is_indeterminate));
        autostartAnimation = a.getBoolean(R.styleable.CustomCircularProgressView_cpv_animAutostart,
                resources.getBoolean(R.bool.cpv_default_anim_autostart));
        initialStartAngle = a.getFloat(R.styleable.CustomCircularProgressView_cpv_startAngle,
        ...
        if (a.hasValue(R.styleable.CustomCircularProgressView_cpv_color)) {
            color = a.getColor(R.styleable.CustomCircularProgressView_cpv_color, ResourcesCompat
                    .getColor(resources, R
                            .color.cpv_default_color, null));
        }
        ...
        // If using native accentColor (SDK >21)
            ...
            TypedArray t = getContext().obtainStyledAttributes(new int[]{android.R.attr
                    .colorAccent});
            color = t.getColor(0, ResourcesCompat
                    .getColor(resources, R
                            .color.cpv_default_color, null));
        } else {
            //Use default color
        ...
        animDuration = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animDuration,
                resources.getInteger(R.integer.cpv_default_anim_duration));
        animSwoopDuration = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animSwoopDuration,
                resources.getInteger(R.integer.cpv_default_anim_swoop_duration));
        animSyncDuration = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animSyncDuration,
                resources.getInteger(R.integer.cpv_default_anim_sync_duration));
        animSteps = a.getInteger(R.styleable.CustomCircularProgressView_cpv_animSteps,
    ...
    private void updateBounds() {
        ...
        bounds.set(paddingLeft + thickness, paddingTop + thickness, size - paddingLeft -
                thickness, size - paddingTop - thickness);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw the arc
        float sweepAngle = (isInEditMode()) ? currentProgress / maxProgress * 360 :
                actualProgress / maxProgress * 360;
        if (!isIndeterminate)
            canvas.drawArc(bounds, startAngle, sweepAngle, false, paint);
        else
            canvas.drawArc(bounds, startAngle + indeterminateRotateOffset, indeterminateSweep,
                    false, paint);
    }
    ...
    private AnimatorSet createIndeterminateAnimator(float step) {
        ...
        // Overall rotation
        ValueAnimator rotateAnimator1 = ValueAnimator.ofFloat(step * 720f / animSteps, (step +
                .5f) * 720f / animSteps);
        ...
        ValueAnimator backEndRetract = ValueAnimator.ofFloat(start, start + maxSweep -
